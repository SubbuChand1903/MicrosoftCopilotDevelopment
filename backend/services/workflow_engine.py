...
def _parse_nl(q: str):
    """Parse natural language string and return a card."""
    global LAST_CREATED_TICKET_ID
    ql = q.lower()
    ticket_id = extract_ticket_id(q)

    # FOLLOW-UP REFERENCE TO LAST CREATED TICKET
    if any(p in ql for p in ["created above", "created earlier", "created just now", "ticket above", "the above ticket"]):
    ...
    if ticket_id and any(w in ql for w in ["in progress", "start", "begin", "working"]):
    ...
    if create_intent:
        has_details = any(w in ql for w in ["for ", "about ", "assign", "priority", "high", "low", "bug", "feature", "issue"])
        ...
        assign_match = re.search(r'assign(?:ed)? to\s+([A-Za-z\s@.]+?)(?:\s*$|\.|\,)', q, re.IGNORECASE)
        if assign_match:
            assignee = assign_match.group(1).strip()

        desc_match = re.search(r'description[:\s]+(.+?)(?:\s+priority|\s+assign|$)', q, re.IGNORECASE)
        if desc_match:
            description = desc_match.group(1).strip()

        ticket = create_ticket(title=title, description=description, priority=priority, assignee=assignee, category=category)
    ...
    assign_filter = re.search(r'(?:assigned to|tickets for|tickets by)\s+([A-Za-z\s@.]+?)(?:\s*$|\.)', q, re.IGNORECASE)
    if assign_filter:
        from cards.ticket_board import ticket_board_card
        return ticket_board_card(filter_assignee=assign_filter.group(1).strip())

    if any(w in ql for w in ["board", "all ticket", "my ticket", "show ticket", "list ticket", "view ticket", "get ticket", "fetch ticket", "show me ticket", "can you show"]):
        from cards.ticket_board import ticket_board_card
        return ticket_board_card()
...
def handle_workflow(query):
    ...
    # --------------------------------------------------
    # STEP 1: Normalize {"text": "..."} into action dict
    # --------------------------------------------------
    if isinstance(query, dict) and "text" in query and "action" not in query:
        ...
        elif any(p in ql for p in ["show all tickets", "show tickets", "list tickets", "view board", "back to dashboard", "back to board", "refresh"]):
    ...
    if isinstance(query, dict) and "action" in query:
        ...
        if action == "create_ticket_submit":
